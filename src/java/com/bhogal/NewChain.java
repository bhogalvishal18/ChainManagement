...
//get chain
   ...
   if(isNullOrEmpty(parent_refer_code))
   {
       ...
       PreparedStatement preparedStmtchain = (PreparedStatement) con.prepareStatement("WITH RECURSIVE chain_list (username, refer_code,level, path) AS\n" +
"(\n" +
...
"    username='"+username+"'\n" +
"  UNION ALL\n" +
"  SELECT c.username, c.refer_code,cp.level + 1, CONCAT(cp.path, ' > ', c.username)\n" +
